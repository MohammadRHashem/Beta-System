...
const AlfaTrustPage = () => {
    ...
    }, [pagination.page, pagination.limit, filters, debouncedSearch]); // Added debouncedAmountSearch to dependencies
    ...
    }, [debouncedSearch, filters.dateFrom, filters.dateTo, filters.operation]);
        ...
        <>
            <PageContainer>
                <Header>
                    ...
                    {/* Buttons are now just simple placeholders, as their handlers were removed */}
                    <div style={{ display: 'flex', gap: '1rem' }}>
                        <SyncButton onClick={() => triggerAlfaSync().then(() => alert('Sync triggered!'))}><FaSyncAlt /> Refresh Data</SyncButton>
                        <Button style={{backgroundColor: '#1D6F42'}} onClick={() => exportAlfaExcel({ ...filters, search: debouncedSearch })}><FaFileExcel /> Export Excel</Button>
                        <Button onClick={() => setIsExportModalOpen(true)}><FaFilePdf/> Export PDF</Button>
                    </div>
                </Header>
                {hasNewData && (
                    <RefreshBanner onClick={() => fetchTransactions(pagination.page, filters, debouncedSearch)}>
