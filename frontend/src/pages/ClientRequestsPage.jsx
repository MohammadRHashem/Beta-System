...
const HistoryBadge = styled.span`
    ...
    font-weight: 700;
    border: 1px solid;
    &.new { background: #eefbf3; color: #146c43; border-color: #bce8ca; }
    &.history { background: #eef3ff; color: #1d3f91; border-color: #bfd0fb; }
`;
const HistoryBadgeButton = styled.button`
    padding: 0;
    border: none;
...
const ClientRequestsPage = () => {
    ...
    const canEditSettings = hasPermission('settings:edit_request_triggers');
    ...
    const openHistoryModal = async (request) => {
        const historyCount = Number(request.history_completed_count) || 0;
        if (historyCount <= 0) return;

        setIsHistoryModalOpen(true);
        setHistoryLoading(true);
        setHistoryPayload({
            content_label: request.content_label || null,
        ...
        <>
            ...
            <Modal isOpen={isHistoryModalOpen} onClose={() => setIsHistoryModalOpen(false)}>
                ...
                <HistoryHint>
                    ...
                    <strong>{historyPayload?.content || ''}</strong>
                </HistoryHint>
                {historyLoading ? (
