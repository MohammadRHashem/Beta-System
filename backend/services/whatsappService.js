...
const findAlfaTrustMatchInDb = async (invoiceJson) => {
  ...
  try {
    ...
    console.error(
      "[DB-CONFIRM-ERROR] Error querying alfa_transactions table:",
      dbError,
    );
...
// =======================================================================
const findBestTrkbitMatch = async (
  searchAmount,
  searchSender,
  pixKeyOptions,
) => {
  ...
  const ocrWords = new Set(
    ocrNameNormalized.split(" ").filter((w) => w.length > 1),
  );
  ...
  for (const tx of candidates) {
    ...
    if (
      dbNameNormalized.includes(ocrNameNormalized) ||
      ocrNameNormalized.includes(dbNameNormalized)
    ) {
    ...
    const dbWords = new Set(
      dbNameNormalized.split(" ").filter((w) => w.length > 1),
    );
    const commonWordsCount = [...ocrWords].filter((word) =>
      dbWords.has(word),
    ).length;
    ...
    if (
      shorterWordCount > 0 &&
      commonWordsCount / shorterWordCount >= 0.6 &&
      commonWordsCount >= 1
    ) {
      console.log(
        `[TRKBIT-MATCH] Found via Vector 3 (Word Intersection): ${tx.id}`,
      );
      return tx.id;
    }
  }
  ...
  try {
    ...
    for (const tx of foundTxs) {
      ...
      const commonWords = new Set(
        [...ocrWords].filter((word) => dbWords.has(word)),
      );

      const shorterWordCount = Math.min(ocrWords.size, dbWords.size);
      if (
        shorterWordCount > 0 &&
        commonWords.size / shorterWordCount >= 0.6 &&
        commonWords.size >= 1
      ) {
        return tx.id;
      }
...
const findBestXPayzMatch = async (
  searchAmount,
  searchSender,
  subaccountPool = [],
) => {
  ...
  try {
    ...
    const ocrNameNormalized = normalizeNameForMatching(searchSender);
    const ocrWords = new Set(
      ocrNameNormalized.split(" ").filter((w) => w.length > 2),
    ); // Filter tiny words

    for (const tx of foundTxs) {
      // Fallback: If DB didn't normalize on insert, normalize now
      const dbNameNormalized =
        tx.sender_name_normalized || normalizeNameForMatching(tx.sender_name);
      ...
      const dbWords = new Set(
        dbNameNormalized.split(" ").filter((w) => w.length > 2),
      );
      ...
      if (
        ...
        matchCount / totalSignificantWords >= 0.6 &&
        matchCount >= 1
      ) {
        console.log(
          `[XPAYZ-MATCH] Found via Vector C (Intersection): ${tx.id} (${matchCount}/${totalSignificantWords} words)`,
        );
        return tx.id;
      }
    }
...
const findBestUsdtMatch = async (searchAmount, recipientAddress) => {
  ...
  try {
    ...
    console.error(
      "[DB-CONFIRM-ERROR] Error querying usdt_transactions table:",
      dbError,
    );
    return null;
  }
  ...
  try {
    ...
    if (link) {
      ...
      try {
        ...
        console.warn(
          "[MANUAL-API] Could not react to forwarded message:",
          e.message,
        );
      }
    }
    ...
    console.error(
      `[MANUAL-API-ERROR] Failed to send confirmation for ${originalMessageId}:`,
      error,
    );
    throw error;
  }
...
const sendManualRejection = async (originalMessageId) => {
  try {
    ...
    const [[invoice]] = await pool.query(
      "SELECT media_path FROM invoices WHERE message_id = ?",
      [originalMessageId],
    );
    if (
      invoice &&
      invoice.media_path &&
      fsSync.existsSync(invoice.media_path)
    ) {
    ...
    console.error(
      `[MANUAL-API-ERROR] Failed to send rejection for ${originalMessageId}:`,
      error,
    );
    throw error;
  }
...
const reactToMessage = async (messageId, reaction) => {
  try {
    ...
    console.warn(
      `[REACTION-APPLY] Could not apply reaction to message ${messageId}:`,
      error.message,
    );
  }
};
...
const fetchExchangeRate = async () => {
  const apiUrl =
    process.env.RATE_API_URL || "https://api.fxratesapi.com/latest";
  try {
    ...
    console.error(
      "[TAXA] API response was successful but did not contain the BRL rate.",
    );
    return null;
  } catch (error) {
    console.error(
      "[TAXA] Failed to fetch exchange rate from API:",
      error.message,
    );
    return null;
  }
...
const invoiceWorker = new Worker(
  ...
  async (job) => {
    ...
    const filename =
      originalMessage.filename || originalMessage._data?.filename;

    // 2. Log the critical properties for debugging every time.
    console.log(
      `[WORKER-DEBUG] Message Type: ${originalMessage.type}, Filename: "${filename}"`,
    );
    ...
    try {
      ...
      if (
        ...
        if (assignedSubaccount) {
          ...
          console.log(
            `[CROSS-CONFIRM] Path A: Group is assigned to PIX Key: ${expectedPixKey}`,
          );

          confirmedTxId = await findBestTrkbitMatch(
            searchAmount,
            searchSender,
            { mode: "include", keys: [expectedPixKey] },
          );
          ...
          console.log(
            `[CROSS-CONFIRM] Path B: Group is unassigned. Searching in the public pool.`,
          );
          ...
          confirmedTxId = await findBestTrkbitMatch(
            searchAmount,
            searchSender,
            { mode: "exclude", keys: exclusionList },
          );
        ...
        if (confirmedTxId) {
          ...
          if (updateResult.affectedRows > 0) {
            const [[{ uid }]] = await pool.query(
              "SELECT uid FROM trkbit_transactions WHERE id = ?",
              [confirmedTxId],
            );
            ...
            try {
              ...
              if (subaccount && subaccount.account_type === "cross") {
                console.log(
                  `[WORKER] Source group is a 'cross' subaccount. Skipping informative forward.`,
                );
              } else {
                let destJid = null;
                ...
                if (directRule) {
                  ...
                  const [rules] = await pool.query(
                    "SELECT trigger_keyword, destination_group_jid FROM forwarding_rules WHERE is_enabled = 1",
                  );
                  for (const rule of rules) {
                    if (
                      recipientNameLower.includes(
                ...
                if (destJid) {
                  ...
                  console.log(
                    `[TRKBIT-INFO] Informative forward sent to ${destJid}`,
                  );
                }
              }
            } catch (infoError) {
              console.error(
                "[TRKBIT-INFO-ERROR] Failed to send informative forward:",
                infoError,
              );
            }
...
invoiceWorker.on("failed", (job, err) =>
  console.error(`[QUEUE-FAIL] Job ${job?.id} failed: ${err.message}`),
);
invoiceWorker.on("completed", (job) =>
  console.log(`[QUEUE-SUCCESS] Job ${job.id} has completed.`),
);
...
const refreshTrocaCoinStatus = async () => {
  try {
    ...
    isTrocaCoinTelegramEnabled = setting
      ? setting.setting_value === "true"
      : false;
    console.log(
      `[SETTINGS] Troca Coin Telegram Confirmation is now ${isTrocaCoinTelegramEnabled ? "ENABLED" : "DISABLED"}.`,
    );
  } catch (error) {
    console.error(
      "[SETTINGS-ERROR] Failed to refresh Troca Coin status:",
      error,
    );
...
const refreshAlfaApiConfirmationStatus = async () => {
  try {
    ...
    isAlfaApiConfirmationEnabled = setting
      ? setting.setting_value === "true"
      : false;
    console.log(
      `[SETTINGS] Alfa API Confirmation is now ${isAlfaApiConfirmationEnabled ? "ENABLED" : "DISABLED"}.`,
    );
  } catch (error) {
    console.error(
      "[SETTINGS-ERROR] Failed to refresh Alfa API confirmation status:",
      error,
    );
...
const refreshAutoConfirmationStatus = async () => {
  try {
    ...
    isAutoConfirmationEnabled = setting
      ? setting.setting_value === "true"
      : false;
    console.log(
      `[SETTINGS] Auto Confirmation is now ${isAutoConfirmationEnabled ? "ENABLED" : "DISABLED"}.`,
    );
  } catch (error) {
    console.error(
      "[SETTINGS-ERROR] Failed to refresh auto confirmation status:",
      error,
    );
...
const reconcileMissedMessages = async () => {
  if (isReconciling) {
    console.log(
      "[RECONCILER] Missed message reconciliation already in progress. Skipping.",
    );
    return;
  }
  if (connectionStatus !== "connected") {
  ...
  isReconciling = true;
  console.log(
    "[RECONCILER] Starting check for missed messages from the last 10 hours.",
  );
  ...
  try {
    ...
    if (allRecentMessageIds.size === 0) {
      console.log(
        "[RECONCILER] No recent media messages found. Check complete.",
      );
      isReconciling = false;
      return;
    }
    ...
    const missedMessageIds = [...allRecentMessageIds].filter(
      (id) => !processedIds.has(id),
    );

    if (missedMessageIds.length > 0) {
      console.log(
        `[RECONCILER] Found ${missedMessageIds.length} missed messages. Queuing them now.`,
      );
      for (const messageId of missedMessageIds) {
        await queueMessageIfNotExists(messageId);
      }
      console.log(
        `[RECONCILER] Successfully queued ${missedMessageIds.length} missed jobs.`,
      );
    } else {
...
const reconcileStalledJobs = async () => {
  ...
  console.log(
    "[STALLED-RECONCILER] Starting check for jobs that are truly stalled in the queue.",
  );
  ...
  try {
    ...
    const stalledJobs = await invoiceQueue.getJobs(
      ["active", "waiting"],
      0,
      100,
      true,
    ...
    for (const job of stalledJobs) {
      ...
      if (job.timestamp < fiveMinutesAgo.getTime()) {
        try {
          console.log(
            `[STALLED-RECONCILER] Found a potentially stalled job: ${job.id}. State: ${await job.getState()}`,
          );
          ...
          await invoiceQueue.add(
            "process-invoice",
            { messageId: job.id },
            { jobId: job.id, removeOnComplete: true, removeOnFail: 50 },
          );
          requeuedCount++;
          console.log(
            `[STALLED-RECONCILER] Successfully re-queued job ${job.id}.`,
          );
        } catch (error) {
          console.error(
            `[STALLED-RECONCILER] Failed to re-queue job ${job.id}:`,
            error.message,
          );
        }
      }
    ...
    if (requeuedCount > 0) {
      console.log(
        `[STALLED-RECONCILER] Finished check. Re-queued ${requeuedCount} stalled jobs.`,
      );
    } else {
      console.log(
        "[STALLED-RECONCILER] Finished check. No stalled jobs found in the queue.",
      );
    }
  } catch (error) {
    console.error(
      "[STALLED-RECONCILER-ERROR] A critical error occurred during stalled job reconciliation:",
      error,
    );
  } finally {
...
const auditAndReconcileInternalLog = async () => {
  ...
  try {
    ...
    if (missedMessages.length > 0) {
      console.log(
        `[AUDITOR] Found ${missedMessages.length} messages that were received but not queued. Processing them now.`,
      );
    ...
    console.error(
      "[AUDITOR-ERROR] A critical error occurred during the internal audit:",
      error,
    );
  } finally {
    isAuditing = false;
...
const queueMessageIfNotExists = async (messageId, options = {}) => {
  ...
  try {
    ...
    if (isProcessed) {
      ...
      if (options.forceIfMissingInvoice) {
        const [invoiceRows] = await connection.query(
          "SELECT id FROM invoices WHERE message_id = ? AND is_deleted = 0",
          [messageId],
        );
        ...
        if (invoiceRows.length > 0) {
          console.log(
            `[QUEUE-SKIP] Force retry ignored for ${messageId}. Invoice already exists.`,
          );
        ...
        console.log(
          `[QUEUE-FORCE] Message ${messageId} found in processed logs but NOT in invoices. Forcing reprocessing.`,
        );
      } else {
    ...
    await invoiceQueue.add(
      "process-invoice",
      { messageId, ...options },
      { jobId: messageId, removeOnComplete: true, removeOnFail: 50 },
    );
    ...
    if (error.code !== "ER_DUP_ENTRY") {
      console.error(
        `[QUEUE-ERROR] Failed to transactionally queue message ${messageId}. It will be retried by the reconciler.`,
        error,
      );
    }
  } finally {
...
const reconcileDeletedMessages = async () => {
  if (isReconcilingDeletions) {
    console.log(
      "[DELETE-RECONCILER] Deletion reconciliation already in progress. Skipping.",
    );
    return;
  }
  if (connectionStatus !== "connected") {
  ...
  console.log(
    "[DELETE-RECONCILER] Starting proactive check for deleted messages.",
  );
  ...
  try {
    ...
    for (const invoice of invoicesToCheck) {
      try {
        ...
        console.warn(
          `[DELETE-RECONCILER] Could not fetch status for message ${invoice.message_id}. It might be too old or an error occurred.`,
          error.message,
        );
      }
    }
    ...
    if (idsToMarkAsDeleted.length > 0) {
      console.log(
        `[DELETE-RECONCILER] Found ${idsToMarkAsDeleted.length} invoices that were deleted but not marked. Updating now.`,
      );
      await pool.query(
      ...
      console.log(
        "[DELETE-RECONCILER] All recent invoices are in sync. No inconsistencies found.",
      );
    }
  } catch (error) {
    console.error(
      "[DELETE-RECONCILER-ERROR] A critical error occurred during deletion reconciliation:",
      error,
    );
  } finally {
    isReconcilingDeletions = false;
...
const refreshRequestTypeCache = async () => {
  try {
    ...
    console.log(
      `[CACHE] Loaded ${requestTypeCache.length} active request types.`,
    );
  } catch (error) {
    console.error(
      "[CACHE-ERROR] Failed to refresh request types cache:",
      error,
    );
  }
};
...
const clearReaction = async (messageId) => {
  try {
    ...
    console.warn(
      `[REACTION-CLEAR] Could not clear reaction for message ${messageId}:`,
      error.message,
    );
  }
};
    ...
    if (message.body) {
      ...
      if (messageBody === "/taxa") {
        ...
        if (rate !== null) {
          ...
          const errorReply =
            "⚠️ Could not fetch the exchange rate at this time.";
      ...
      for (const rule of requestTypeCache) {
        ...
        if (match && match[1]) {
          ...
          if (ourWallet) {
            console.log(
              `[REQUEST-DETECT] Content is one of our own wallets. Ignoring.`,
            );
            await pool.query(
              "INSERT IGNORE INTO processed_messages (message_id) VALUES (?)",
              [message.id._serialized],
      ...
      //usdt link detection
      const tronScanRegex =
        /https?:\/\/(?:www\.)?tronscan\.org\/#\/transaction\/([a-fA-F0-9]+)/;
...
const handleMessageRevoke = async (message, revoked_msg) => {
  ...
  if (!deletedMessageId) {
    console.warn(
      "[DELETE] Revoke event received but could not determine message ID.",
    );
    return;
  }
  console.log(
    `[DELETE] Real-time revoke event for message ID: ${deletedMessageId}`,
  );
  try {
    const [updateResult] = await pool.query(
...
const handleReaction = async (reaction) => {
  ...
  if (reactionEmoji === "🗑" || reactionEmoji === "🗑️") {
    try {
      ...
      if (msg && msg.fromMe) {
        ...
        console.log(
          `[REACTION-DELETE] Deleted message ${reactedMessageId} for everyone.`,
        );
      } else {
        console.log(
          `[REACTION-DELETE] Ignored delete request on message ${reactedMessageId} (Not from me).`,
        );
      }
    } catch (e) {
      console.error(
        `[REACTION-DELETE-ERROR] Could not delete message:`,
        e.message,
      );
    }
  ...
  if (reactionEmoji === "🔵") {
    console.log(
      `[MANUAL-OVERRIDE] Blue circle detected on ${reactedMessageId}.`,
    );
    try {
      const msg = await client.getMessageById(reactedMessageId);
      if (msg && msg.hasMedia) {
  ...
  try {
    ...
    if (invoiceDetails && invoiceDetails.raw_json_data) {
      try {
        const jsonData =
          typeof invoiceDetails.raw_json_data === "string"
        ...
        console.warn(
          `[REACTION] Could not parse raw_json_data for message ${link.original_message_id}`,
        );
      }
    }
    ...
    if (reactionEmoji === "👍" || reactionEmoji === "✅") {
      ...
      if (originalMessage) {
        ...
        console.log(
          `[REACTION] Successfully processed '${confirmMessage}' confirmation for ${link.original_message_id}`,
        );
      ...
      try {
        ...
        if (invoiceToDelete) {
          ...
          if (
            invoiceToDelete.media_path &&
            fsSync.existsSync(invoiceToDelete.media_path)
          ) {
        ...
        console.log(
          `[REACTION-DELETE] Successfully processed '${rejectMessage}' deletion for ${link.original_message_id}`,
        );
      } catch (dbError) {
        await connection.rollback();
        console.error(
          `[REACTION-DELETE-ERROR] Database transaction failed for ${link.original_message_id}:`,
          dbError,
        );
      } finally {
        connection.release();
    ...
    console.error(
      `[REACTION-ERROR] Failed to process reaction for ${reactedMessageId}:`,
      error,
    );
  }
};
...
const refreshTrocaCoinMethod = async () => {
  try {
    ...
    console.log(
      `[SETTINGS] Troca Coin Confirmation Method is now set to '${trocaCoinConfirmationMethod}'.`,
    );
  } catch (error) {
    console.error(
      "[SETTINGS-ERROR] Failed to refresh Troca Coin method:",
      error,
    );
...
const initializeWhatsApp = (socketIoInstance) => {
    ...
    console.log(
      "[STALLED-RECONCILER] Stalled job reconciler scheduled to run every 5 minutes.",
    );

    cron.schedule("*/15 * * * *", reconcileDeletedMessages);
    console.log(
      "[DELETE-RECONCILER] Proactive deletion-checking reconciler scheduled to run every 15 minutes.",
    );

    console.log(
      "[RECONCILER] Scheduling a check for missed messages in 90 seconds...",
    );
    setTimeout(() => {
      console.log(
        "[RECONCILER] Starting post-connection check for missed messages.",
      );
    ...
    if (!cron.getTasks().length) {
      console.log(
        "[STARTUP] First time startup detected. Clearing any old/stale jobs from the queue...",
      );
      await invoiceQueue.obliterate({ force: true });
      console.log("[STARTUP] Job queue cleared.");
    }
...
const fetchAllGroups = async () => {
  if (connectionStatus !== "connected" || !client.info) {
    throw new Error(
      "WhatsApp is not connected or client info is not available yet.",
    );
  }
  try {
    ...
    const activeGroups = allGroups.filter(
      (group) =>
        group.participants &&
        group.participants.some((p) => p.id._serialized === selfId),
    );
...
const broadcast = async (
  ...
  if (connectionStatus !== "connected") {
    if (socketId)
      localIo
        .to(socketId)
        .emit("broadcast:error", { message: "WhatsApp is not connected." });
    return;
  }
...
const pinMessageToGroups = async ({
  groupObjects,
  message,
  attachment = null,
  durationSeconds,
  ...
  for (const group of groupObjects) {
    ...
    try {
      if (socketId && localIo) {
        localIo.to(socketId).emit("pin:progress", {
          groupName: group.name,
          status: "sending",
          message: `Pinning in "${group.name}"...`,
        });
      }

      if (attachment && attachment.filepath) {
        const media = MessageMedia.fromFilePath(attachment.filepath);
        sentMessage = await client.sendMessage(group.id, media, {
          caption: message || "",
        });
      } else {
        sentMessage = await client.sendMessage(group.id, message || "");
      }

      if (!sentMessage || typeof sentMessage.pin !== "function") {
        throw new Error(
          "Pin message is not supported by whatsapp-web.js in this environment.",
        );
      }

      const pinned = await sentMessage.pin(durationSeconds);
      if (!pinned) {
        throw new Error(
          "Pin failed (missing admin permission or unsupported chat).",
        );
      }
      ...
      results.push({
        ...
        status: "failed",
        messageId:
          typeof sentMessage?.id?._serialized === "string"
      ...
      if (socketId && localIo) {
        localIo.to(socketId).emit("pin:progress", {
          groupName: group.name,
          status: "failed",
          message: `Failed to pin in "${group.name}". Reason: ${error.message}`,
        });
      }
    }
  }
  ...
  if (socketId && localIo) {
    const successful = results.filter((r) => r.status === "pinned").length;
    const failed = results.filter((r) => r.status === "failed").length;
    localIo.to(socketId).emit("pin:complete", {
