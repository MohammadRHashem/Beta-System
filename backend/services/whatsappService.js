...
const invoiceWorker = new Worker(
  ...
  async (job) => {
    ...
    try {
      ...
      if (
        !groupSettings.forwarding_enabled &&
        !groupSettings.archiving_enabled
      ) {
        await connection.commit();
        return;
      }
      ...
      if (groupSettings.forwarding_enabled) {
        ...
        if (recipientNameLower) {
          ...
          for (const rule of rules) {
            if (
              recipientNameLower.includes(rule.trigger_keyword.toLowerCase())
            ) {
      ...
      console.error(
        `[WORKER-ERROR] Critical error processing job ${messageId}:`,
        error
      );
      throw error;
    } finally {
  ...
  try {
    ...
    if (message.hasMedia && !message.fromMe) {
      try {
        ...
        console.log(
          `[QUEUE] Added media from "${chat.name}" to queue. Msg ID: ${messageId}`
        );
      } catch (queueError) {
        console.error(
          `[QUEUE-ERROR] Failed to add message ${messageId} to the queue:`,
          queueError
        );
        await pool.query(
