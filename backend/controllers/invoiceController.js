...
exports.createInvoice = async (req, res) => {
    ...
    try {
        ...
        const now_as_sort = new Date().getTime();
        ...
        if (insertAfterId) {
            ...
            if (insertAfterId === 'START') {
                ...
                final_sort_order = min_sort ? parseFloat(min_sort) - 1 : now_as_sort - 10000;
            } else {
                const [[prevInvoice]] = await connection.query('SELECT sort_order FROM invoices WHERE id = ?', [insertAfterId]);
                if (!prevInvoice) throw new Error(`Previous invoice for sorting (ID: ${insertAfterId}) not found.`);
                
                const prevSort = parseFloat(prevInvoice.sort_order);
                ...
                let nextSort;
                if (nextInvoice) {
                    nextSort = parseFloat(nextInvoice.sort_order);
                    console.log(`[SORT] Found next invoice sort_order: ${nextSort}`);
                } else {
                    nextSort = prevSort + 1; // Add 1 to the previous sort order
                    console.log(`[SORT] No next invoice found. Creating new sort_order at the end: ${nextSort}`);
                }
                
                final_sort_order = prevSort + (nextSort - prevSort) / 2;
