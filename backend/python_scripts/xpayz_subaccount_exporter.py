...
class XPayzClient:
    ...
    def iter_transactions(self, subaccount_id: int, per_page: int = 200, historical: bool = False) -> t.Iterator[Transaction]:
        ...
        while True:
            ...
            # print(f"[XPAYZ-PYTHON] Fetching page {page} for subaccount {subaccount_id}...")
            resp = self._request_with_retries("GET", url, params={"page": page, "per_page": per_page})
            blob = resp.json()
            items = blob.get("data", []) or []
            
            if not items:
                if historical: print(f"[XPAYZ-PYTHON] Last page reached for subaccount {subaccount_id}.")
                break

            for item in items:
                yield self._to_transaction(item)
            
            if not historical: break
            page += 1
            time.sleep(0.5)
    ...
    for tx in transactions:
        ...
        try:
            ...
            values_to_insert.append((
                tx.id, subaccount_id, amount, tx.operation_direct,
                tx.sender_name, normalized, counterparty, tx_date,
                json.dumps(tx.raw), tx.external_id
            ))
